...
const rawInput = `ceg gedcfb ec eabfdg gcdabe baged cabgf gbaec fecagdb eacd | efcgbad adfecbg gec abgce
...
const sortChars = (text) => text.split('').sort().join('');

const input = rawInput
  .split('\n')
  .map((e) => e.split(' | ').map((f) => f.split(' ').map((g) => sortChars(g))));

const buildMap = (pattern) => {
  const alpha = pattern.find((e) => e.length === 2).split('');
  // const beta = pattern
  //   .find((e) => e.length === 3)
  //   .split('')
  //   .filter((e) => !alpha.includes(e));
  const gamma = pattern
    .find((e) => e.length === 4)
    .split('')
    .filter((e) => !alpha.includes(e));
  ...
  pattern.forEach((key) => {
    if (!(key in map)) {
      if (key.length === 2) {
        ...
        if (alpha.every((e) => key.includes(e))) {
          // All alpha included
          map[key] = 3;
        } else if (gamma.every((e) => key.includes(e))) {
          // All gamma included
          map[key] = 5;
        } else {
          map[key] = 2;
        ...
        if (!gamma.every((e) => key.includes(e))) {
          // Not all gamma included
          map[key] = 0;
        } else if (alpha.every((e) => key.includes(e))) {
          // All alpha and gamma included
          map[key] = 9;
        } else {
          map[key] = 6;
        }
...
const convertedSignals = input.map(([pattern, signals]) => {
  const map = buildMap(pattern);
  return Number(signals.map((segment) => map[segment]).join(''));
});
const result = convertedSignals.reduce((a, b) => a + b);
